...
from django.contrib.auth.modesls import User
...
def inquiry(request):
    if request.method == "POST":
        ...
        admin_email = admin_info.mail
        send_mail(
                'New Car Enquiry',
                'You have a new enquiry for the car: ' + car_title + '. Please login to your admin panel to see more info.',
                'from@example.com',
                [admin_email],
                fail_silently=False,
            )
        ...
        contact.save()
        message.sucess(request, 'Your request has been submitted, we will get back to you shortly.')
        return redirect('/cars/'+car_id)
